...
Run from any ROS 2 Humble workspace (no package install needed):

    source /opt/ros/humble/setup.bash
    python3 shelfbot_integration_test.py

Requirements: rclpy is present (part of any ros-humble-ros-base install).
No colcon build required — this is a standalone script.
...
# ─────────────────────────────────────────────────────────────────────────────
# QoS profiles  (must match firmware publishers exactly)
# ─────────────────────────────────────────────────────────────────────────────
...
# Configuration
# ─────────────────────────────────────────────────────────────────────────────
RECEIVE_TIMEOUT_S    = 8.0   # seconds to wait for first message on each topic
HEARTBEAT_WINDOW_S   = 3.0   # seconds to collect heartbeat samples
EPOCH_MIN            = 1_700_000_000  # Nov 2023 — below this = boot-relative stamp
MOTOR_COUNT_EXPECTED = 5
SENSOR_COUNT_EXPECTED = 6    # 4 ultrasonic + 1 tof + 1 lidar
...
# ─────────────────────────────────────────────────────────────────────────────
# Collector node — subscribes to everything and collects messages
# ─────────────────────────────────────────────────────────────────────────────
class Collector(Node):
    def __init__(self):
        ...
        # Storage: topic_shortname → list of received messages
        ...
        # ── Publisher for LED round-trip test ─────────────────────────────
...
def check_node_visible(node: Node) -> bool:
    ...
    visible = "shelfbot_firmware" in names
    record("shelfbot_firmware node is visible", visible,
           f"nodes found: {names}" if not visible else "")
    return visible
...
def check_qos_profiles(node: Node) -> None:
    ...
    # We use get_publishers_info_by_topic to inspect what the firmware advertises.
    ...
    for topic, want in expected.items():
        ...
        if not infos:
            ...
            continue
        got = infos[0].qos_profile.reliability.name  # 'RELIABLE' or 'BEST_EFFORT'
        record(f"QoS {topic.split('/')[-1]} == {want}", got == want,
...
def check_topic_receipt(col: Collector) -> None:
    ...
    for key in ["heartbeat", "motor_positions", "distance_sensors",
        ...
        record(f"{key} receives messages", arrived,
               f"no message in {RECEIVE_TIMEOUT_S} s" if not arrived else
               f"{len(col.get(key))} message(s) received")
...
def check_heartbeat_increments(col: Collector) -> None:
    ...
    if len(msgs) < 2:
        record("Heartbeat increments", False,
...
def check_distance_sensors_payload(col: Collector) -> None:
    ...
    record(f"distance_sensors has {SENSOR_COUNT_EXPECTED} elements",
           cnt == SENSOR_COUNT_EXPECTED, f"got {cnt}")

    # Valid entries are either -1.0 (invalid sensor) or a positive cm value
    valid_values = all(v == -1.0 or 0.0 <= v <= 5000.0 for v in m.data)
    record("distance_sensors values in range or -1.0", valid_values,
           f"values={[round(v,1) for v in m.data]}")
...
def check_tof_distance_payload(col: Collector) -> None:
    ...
    record("tof_distance is -1.0 (no sensor) or in 0–12 m range",
           v == -1.0 or 0.0 <= v <= 12.0, f"value={v:.4f} m")
...
def check_laser_scan_payload(col: Collector) -> None:
    ...
    # range_min / range_max
    record("laser_scan range_min == 0.02 m",  abs(m.range_min - 0.02) < 1e-4,
           f"got {m.range_min}")
    record("laser_scan range_max == 12.0 m",  abs(m.range_max - 12.0) < 1e-4,
           f"got {m.range_max}")

    # Timestamp epoch check
    stamp_sec = m.header.stamp.sec + m.header.stamp.nanosec * 1e-9
    delta     = abs(stamp_sec - host_epoch)

    record("laser_scan stamp is wall-clock epoch (not boot-relative)",
           stamp_sec > EPOCH_MIN,
           f"stamp={stamp_sec:.3f}  (below {EPOCH_MIN} = boot-relative)" if stamp_sec <= EPOCH_MIN
           else f"stamp={stamp_sec:.3f}")

    record("laser_scan stamp within 5 s of host clock",
           delta < 5.0,
           f"|stamp - host| = {delta:.3f} s  (host={host_epoch:.3f})")

    # Range values: must be ≤ range_max + 1.0 (firmware sentinel for invalid)
    ...
    record("laser_scan range values are within bounds",
           valid_ranges,
           f"ranges={[round(r,3) for r in m.ranges]}")
...
def check_no_qos_incompatibility(node: Node) -> None:
    ...
    # motor_positions: firmware BEST_EFFORT pub, hardware_interface BEST_EFFORT sub → ✓
    # laser_scan:      firmware RELIABLE pub,    lidar_relay_node RELIABLE sub       → ✓
    pairs = {
    ...
    for topic, (want_pub, want_sub) in pairs.items():
        ...
        # Filter out our own test subscriber
        remote_subs = [s for s in sub_infos
                       if s.node_name != "shelfbot_test_collector"]

        if not remote_subs:
            # No external subscriber present — just verify publisher side
            record(f"{topic.split('/')[-1]} publisher QoS == {want_pub}",
                   pub_qos == want_pub, f"got {pub_qos}")
            continue
...
def main() -> int:
    ...
    print(f"{BOLD}╚══════════════════════════════════════════════════╝{RESET}")
    print(f"  Timeout per topic : {RECEIVE_TIMEOUT_S} s")
    print(f"  Host epoch        : {time.time():.3f}")
    print(f"  Epoch lower bound : {EPOCH_MIN}  (Nov 2023)")
    ...
    # Spin a bit to collect initial messages for all topics
