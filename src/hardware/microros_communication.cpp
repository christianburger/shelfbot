...
void MicroRosCommunication::set_health_timeout(double seconds) {
    if (seconds <= 0.0) {
        log_warn("MicroRosCommunication", "set_health_timeout",
                 "Ignoring non-positive health timeout " + std::to_string(seconds) +
                 "s — keeping default " +
                 std::to_string(max_allowed_interval_.seconds()) + "s");
        return;
    }
    max_allowed_interval_ = rclcpp::Duration::from_seconds(seconds);
    log_zip("UROS", "HTMO", {{"s", seconds}});
}
...
bool MicroRosCommunication::open(const std::string& /*connection_string*/) {
    try {
        ...
        log_zip("UROS", "OPEN", {{"htmo_s", max_allowed_interval_.seconds()}});

        log_info("MicroRosCommunication", "open",
                 "Successfully opened micro-ROS communication. "
                 "health_timeout=" + std::to_string(max_allowed_interval_.seconds()) + "s");
        return true;
    ...
    try {
        ...
        if (!ok) {
            log_zip("UROS", "HLTH", {{"ok", 0}, {"age_s", age_s},
                                     {"lim_s", max_allowed_interval_.seconds()}});
            log_warn("MicroRosCommunication", "is_communication_healthy",
                    "Communication unhealthy — last message " +
                    std::to_string(age_s) + "s ago (limit " +
                    std::to_string(max_allowed_interval_.seconds()) + "s)");
        }
        return ok;
    } catch (const std::runtime_error& e) {
