...
hardware_interface::CallbackReturn FourWheelDriveHardwareInterface::on_init(const hardware_interface::HardwareInfo& info) {
  ...
  log_info("FourWheelDriveHardwareInterface", "on_init", "Resized position and command vectors to " + std::to_string(info_.joints.size()));
...
hardware_interface::CallbackReturn FourWheelDriveHardwareInterface::on_configure(const rclcpp_lifecycle::State& previous_state) {
  ...
  log_debug("FourWheelDriveHardwareInterface", "on_configure", "Previous state: " + std::string(previous_state.label()));
...
std::vector<hardware_interface::StateInterface> FourWheelDriveHardwareInterface::export_state_interfaces() {
  ...
  log_info("FourWheelDriveHardwareInterface", "export_state_interfaces", "Exported " + std::to_string(state_interfaces.size()) + " state interfaces");
...
std::vector<hardware_interface::CommandInterface> FourWheelDriveHardwareInterface::export_command_interfaces() {
  ...
  for (size_t i = 0; i < info_.joints.size(); i++) {
    log_debug("FourWheelDriveHardwareInterface", "export_command_interfaces", "Adding command interface for joint: " + info_.joints[i].name);
    command_interfaces.emplace_back(hardware_interface::CommandInterface(
        info_.joints[i].name, hardware_interface::HW_IF_POSITION, &hw_commands_[i]));
  }
  log_info("FourWheelDriveHardwareInterface", "export_command_interfaces", "Exported " + std::to_string(command_interfaces.size()) + " command interfaces");
...
hardware_interface::return_type FourWheelDriveHardwareInterface::read(const rclcpp::Time& time, const rclcpp::Duration& period) {
  ...
  for (size_t i = 0; i < hw_positions_.size(); ++i) {
    log_debug("FourWheelDriveHardwareInterface", "read", "Joint " + std::to_string(i) + " position: " + std::to_string(hw_positions_[i]));
  }
...
hardware_interface::return_type FourWheelDriveHardwareInterface::write(const rclcpp::Time& time, const rclcpp::Duration& period) {
  ...
  log_debug("FourWheelDriveHardwareInterface", "write", "Time: " + std::to_string(time.seconds()) + ", Period: " + std::to_string(period.seconds()));
